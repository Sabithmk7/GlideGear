...
function Dashboard() {
  ...
  <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-lg p-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div className="bg-blue-50 border border-blue-300 rounded-lg p-6 flex flex-col items-center dashboard-box">
        <h2 className="text-xl font-semibold text-blue-700 mb-2">Total Users</h2>
        <p className="text-lg font-bold text-blue-800 mb-4 overflow-hidden">{totalUsers || 0}</p>
        <div className="w-full max-w-xs">
          <Bar data={userChartData} options={{ responsive: true, plugins: { legend: { display: true } } }} />
        </div>
      </div>
      <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-6 flex flex-col items-center dashboard-box">
        <h2 className="text-xl font-semibold text-yellow-700 mb-2">Total Orders</h2>
        <p className="text-xl font-bold text-yellow-800 mb-4 overflow-hidden">{totalProductsPurchased || 0}</p>
        <div className="w-full max-w-xs">
          <Pie data={orderChartData} options={{ responsive: true, plugins: { legend: { display: true } } }} />
        </div>
      </div>
      <div className="bg-green-50 border border-green-300 rounded-lg p-6 flex flex-col items-center dashboard-box">
        <h2 className="text-xl font-semibold text-green-700 mb-2">Total Products</h2>
        <p className="text-xl font-bold text-green-800 mb-4 overflow-hidden">{totalProducts || 0}</p>
        <div className="w-full max-w-xs">
          <Bar data={productChartData} options={{ responsive: true, plugins: { legend: { display: true } } }} />
        </div>
      </div>
      <div className="bg-red-50 border border-red-300 rounded-lg p-6 flex flex-col items-center dashboard-box">
        <h2 className="text-xl font-semibold text-red-700 mb-2">Total Profit</h2>
        <p className="text-xl font-bold text-red-800 mb-4 overflow-hidden">₹{totalRevenue?.toFixed(2) || 0.00}</p>
        <div className="w-full max-w-xs">
