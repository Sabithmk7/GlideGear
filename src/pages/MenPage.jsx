...
function MenPage() {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        ...
        setRecords(menItems)
        setData(menItems);
        setTotalPages(Math.ceil(menItems.length / itemsPerPage));
      } catch (error) {
  ...
  function handleSearch(e){
    setData(records.filter(value=>value.name.toLowerCase().includes(e.target.value)))
  }

  return (
    <div className="p-4 sm:p-8 md:p-16 lg:p-32 bg-gray-200">
      <div className="py-4 flex justify-between"><h1 className="text-2xl md:text-3xl font-bold ">Men's Products</h1>
